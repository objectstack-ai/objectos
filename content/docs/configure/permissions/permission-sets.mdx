...
- **No shipped set carries the grant — the upgrade is what takes it
  ...
  you author — platform sets are not overridable.
  **Environment-authored sets never carried it either** — edit any
  custom set whose users export. Nothing fails at parse time, so an
  upgrade that edits nothing is valid metadata whose administrators
  have quietly lost export on every object no app set names.
