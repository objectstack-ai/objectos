...
> **警告：**旧的 `referenceFilters: string[]` 属性（如 `['is_active = true']`）**已在 16.0 中移除** —— 解析时会被直接剥离；而且即使在移除之前，记录选择器 UI 也从未读取过它，它什么都不过滤。上面展示的结构化 `lookupFilters` + `dependsOn` 是唯一受支持的形式。
...
**16.0 新增：**可选的 `filter` 是一个查询式 `where` 谓词 —— 只有匹配它
的子记录才会被聚合（例如只统计 `status == 'active'` 的子记录）；当某条
子记录进入或离开该谓词范围时，汇总会重新聚合。它还让多个 summary 字段
可以对*同一个*子对象汇总出不同的总计。
