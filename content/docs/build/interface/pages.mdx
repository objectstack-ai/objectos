...
Analytics-bearing components (`chart`, `metric_card`) bind a named
**dataset** and select its `dimensions` + `values` by name — the same
ADR-0021 shape as [dashboard](/docs/build/interface/dashboards) widgets
and Studio's only output. The removed inline-analytics shape (`object` +
`categoryField` + `valueField` + `aggregate` on the component) is a
parse error since 16.0, not a silently ignored no-op.
