...
> Since 16.0, the widget schema is **strict**: an undeclared top-level key
> — a typo, a hallucinated key, or a removed inline-query key (`object` +
> `categoryField` + `valueField` + `aggregate`, pivot `rowField` /
> `columnField`) — is a **parse error that names the offending key** and
> points at the dataset shape, no longer silently stripped into a widget
> that renders nothing. Define a dataset and bind widgets to it;
> `options` remains the free-form escape hatch for renderer-specific
> extras.
