...
承载分析数据的组件（`chart`、`metric_card`）绑定一个具名**数据集**，并按名称选用其 `dimensions` + `values` —— 与[仪表盘](/docs/build/interface/dashboards)组件相同的 ADR-0021 形态，也是 Studio 唯一会生成的形态。已移除的内联分析形态（在组件上直接写 `object` + `categoryField` + `valueField` + `aggregate`）自 16.0 起是解析错误，而不再是被悄悄忽略的空操作。
