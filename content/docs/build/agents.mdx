...
Copilot Console, and ServiceNow Now Assist:
...
Agentforce style) instead of forcing the user to pick an agent, use
...
1. The **default agent** for the active app (declared on the App
   metadata), or the first agent the user has access to.
2. The **active skills** — filtered by user permission set, current
   object/record context, and the agent's `skills:` list.
3. The **knowledge** attached to the agent.

You don't have to wire which agent is shown where. Declare an app,
mark one of its agents as default, and it appears.
...
- [Actions](/docs/build/actions) — declare the tools your agents will use
