...
**Q: Can I disable Studio / Account / Console and use only the REST API?**
A: Yes. Run `os start --no-ui` or set the corresponding flags. The
REST API is the same whether the UIs are mounted or not.

**Q: Can I use my own front-end instead of Studio?**
A: Yes. Studio uses the same `/api/v1/*` endpoints you'd call from
...
A: CSV: yes, via `os data create` in a loop or the Studio bulk upload.
...
and Studio. See [Permission Sets](/docs/configure/permissions/permission-sets).
...
A: Yes, via MCP (`@objectstack/plugin-mcp-server`) — exposes objects
...
**Q: Can I customize Studio's look?**
