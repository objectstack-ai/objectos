...
(`/api/v1/data/<object>/actions/<action>`), Studio buttons, or flow
...
navigable application in Studio. Multiple apps can coexist in one
...
audit log, sessions, API keys, system settings. Distinct from Studio
(business UI).
...
A typed property on an Object. ~25 built-in types: `text`, `select`,
`lookup`, `markdown`, `attachment`, `formula`, `rollup`, etc. See
...
The in-Studio catalog of installable apps. Backed by a configurable
...
TypeScript schema; generates REST APIs, Studio views, audit entries,
RBAC checkpoints automatically. See [Data Model](/docs/build/data-model).
...
to native SQL / Mongo queries. Used by REST endpoints, Studio,
...
charts, navigation. Studio renders ObjectUI declarations.
...
`plugin-mcp-server`, etc. Activated via DI + lifecycle hooks
(`init → start → destroy`).
...
The business UI at `/_studio/` — browse, create, edit records,
...
`/` (REST API), `/_studio/`, `/_account/`, `/_console/`.
...
attached to an Object. Studio renders it; you don't write the
component.
