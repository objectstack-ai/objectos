...
Object 上の型付きプロパティ。約25種類の組み込み型: `text`、`select`、`lookup`、`markdown`、`attachment`、`formula`、`rollup` など。[Data Model](/docs/build/data-model) を参照してください。
...
ランタイムをある機能で拡張するフレームワークパッケージ — `plugin-auth`、`plugin-security`、`plugin-audit`、`plugin-webhooks`、`plugin-mcp-server` など。DI + ライフサイクルフック（`init → start → destroy`）を介して有効化されます。
