...
That block is transcribed from `@objectstack/cli` 17.4.0 booting an empty
...
boot diagnostics) and the full list of loaded plugin names, both of which
depend on your environment. It also logs two SQL-driver errors that do not stop
the server: a `DATABASE_ERROR` for a missing `_objectstack_sequences` table, and
an `Insert operation failed` on `sys_oauth_resource` with a stack trace.
...
`@objectstack/cli` 17.4.0. Project paths show as `my-app/…` — the CLI prints
...
names, and an MCP connection trailer. A separate process prints that trailer,
so it usually follows `Press Ctrl+C to stop` but can land inside the banner. The
run also logs SQL-driver errors that do not stop the server: a `DATABASE_ERROR`
for a missing `_objectstack_sequences` table (printed twice), and an
`Insert operation failed` on `sys_oauth_resource` with a stack trace.
