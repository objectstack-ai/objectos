...
Where the running app comes from is one decision, and the deployment declares
it. The shipped default needs no configuration; each of the other modes is
selected by naming a published artifact, and the
[Environment Variables](/docs/reference/environment-variables) reference
carries the exact contract for each.
...
any of the modes above. A self-hosted runtime authenticates to a control plane
with a token minted when the deployment was **bound** to it, not with a key
pasted into a file, and pairing the wrong licence mode with the wrong cloud
posture is [refused at startup](/docs/deploy/air-gapped).
