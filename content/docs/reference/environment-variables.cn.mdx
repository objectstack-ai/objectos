...
| 变量 | 是否必需 | 说明 |
|---|---:|---|
| `PORT` | 否 | 容器内的 HTTP 端口。Docker Compose 中默认 `3000`。 |
| `OS_AUTH_SECRET` | 启用 auth 时必需 | 用于派生各项目 auth 密钥的基础密钥。 |
...
| `OS_MULTI_TENANT` | 否 | 设为 `false` 以禁用多租户路由（默认 `true`）。 |
...
| 变量 | 说明 |
|---|---|
| `AUTH_SECRET` | 框架包使用的回退 auth 密钥名。ObjectOS 优先使用 `OS_AUTH_SECRET`。 |
