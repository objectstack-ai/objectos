...
Les endpoints générés automatiquement sont gouvernés par objet, de sorte que
vous pouvez retirer un objet de l'API ou le rendre en lecture seule sans écrire
aucun code de route :

- **`apiEnabled`** (booléen, **par défaut `true`**) — définissez `false` pour
  retirer entièrement l'objet de la surface REST. Les requêtes vers ses routes
  renvoient 404.
- **`apiMethods`** (liste blanche optionnelle) — lorsqu'elle est définie, seules
  les opérations listées sont accessibles ; toute autre opération est rejetée au
  niveau de la couche REST. Valeurs autorisées : `get`, `list`, `create`,
...
Les deux sont désormais appliqués au niveau de la couche REST (ADR-0049) — les
versions antérieures analysaient ces propriétés mais ne les appliquaient pas.
...
- [Build → Actions](/docs/build/interface/actions) — déclarer des points de terminaison personnalisés
