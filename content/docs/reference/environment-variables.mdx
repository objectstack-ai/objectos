...
> `OS_` prefix. Pre-1.0 unprefixed names (`PORT`, `AUTH_SECRET`, `OS_MULTI_TENANT`,
> …) still work but emit a one-shot deprecation warning. Prefer the canonical
> `OS_*` names in new deployments; see [Legacy aliases](#legacy-aliases).
