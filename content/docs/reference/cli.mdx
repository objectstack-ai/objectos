...
1. **Empty boot** — no artifact, no config in cwd → boots a bare
   kernel with Studio + marketplace.
...
os start --no-ui                       # API only (no Studio/Account/Console)
```
...
### `os studio` — Studio with dev server

Same as `os dev` but with Studio explicitly enabled.
...
| `--no-ui` | Disable Studio / Account / Console portals |
